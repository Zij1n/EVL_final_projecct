...
def embed_trajectory_dataset(
    ...
    device=None,
    embed_goal=False,
    batch_size=4,  # Process trajectories in small batches to save memory
):
        ...
        with eval_mode(model, no_grad=True):
            for i in range(0, len(dataset), batch_size):
                ...
                for j in batch_indices:
                    obs, *rest = dataset[j]
                    # Move observation to device in smaller chunks if needed
                    if obs.shape[0] > 50:  # If trajectory is long, process in chunks
                        chunks = []
                        for k in range(0, obs.shape[0], 50):
                            chunk = obs[k : k + 50].to(accelerator.device)
                            chunk_enc = model(chunk).to("cpu")
                            chunks.append(chunk_enc)
                            # Clear CUDA cache to free memory
                            if hasattr(torch.cuda, "empty_cache"):
                                torch.cuda.empty_cache()
                        obs_enc = torch.cat(chunks, dim=0)
                    else:
                        obs = obs.to(accelerator.device)
                        obs_enc = model(obs).to("cpu")

                    if obs_only:
                        batch_results.append(obs_enc)
                    else:
                        if embed_goal:
                        ...
                        batch_results.append((obs_enc, *rest))
