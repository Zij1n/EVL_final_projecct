...
def copy_codebase_to(tgt_dir: Path):
    tgt_dir = Path(tgt_dir)
    try:
        repo_root = (
            sp.run(["git", "rev-parse", "--show-toplevel"], check=True, stdout=sp.PIPE)
            .stdout.decode("utf-8")
            .strip()
        )
        repo_root = Path(repo_root)
    except sp.CalledProcessError as e:
        logging.info(f"Error getting git root: {e}")
        return
    try:
        manifest = (
            sp.run(
        ...
        logging.info(f"Error listing git files: {e}")
        return
