...
class MaskedSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.use_diagonal_mask = config.use_diagonal_mask
        self.diagonal_size = config.diagonal_size
        ...
        # Register the bias mask
        if self.use_diagonal_mask:
            base_mask = torch.eye(config.block_size)
            bias_mask = base_mask.clone()
            for i in range(1, self.diagonal_size):
                bias_mask += torch.roll(base_mask, shifts=-i, dims=1)
            bias_mask = torch.tril(
                bias_mask
            )  # Apply lower triangle to mask future tokens
            bias_mask = (bias_mask > 0).float()  # Ensure binary
        else:
            bias_mask = torch.tril(
        ...
        self.register_buffer(
            "bias",
            bias_mask.view(1, 1, config.block_size, config.block_size),
        )
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(
                ...
                k,
                v,
                attn_mask=self.bias[:, :, :T, :T] if self.use_diagonal_mask else None,
                dropout_p=self.dropout if self.training else 0,
                is_causal=not self.use_diagonal_mask,
            )
        else:
            # manual implementation of attention
            att = (q @ k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))
            att = att.masked_fill(bias_mask == 0, float("-inf"))
